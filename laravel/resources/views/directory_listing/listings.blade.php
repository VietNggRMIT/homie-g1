...
<button type="button"
    ...
    <a href={{ route('route_home') }}>Go back to Home</a>
</button>
...
<hr>
    ...
    @foreach ($listings_true as $listing)
        ...
        <h1 class="text-4xl text-lime-900 hover:text-lime-500"><a href={{ route('listings.show', ['listing' => $listing->id]) }}>Listing name: {{ $listing->listing_name }}</a></h1>
        <div {!! $listing->listing_available == 0 ? 'style="opacity: 0.4"' : 'style="opacity: 1"' !!}>
            ...
            <div class="flex">
                @foreach ($listing->listingimages as $listingimage)
                    <img src="{{ asset('storage/images/').'/'.$listingimage->listing_image_path }}" class="object-contain h-48 w-96">
                @endforeach
            </div>
            ...
            <div>Posted by: <a href={{ route('users.show', ['user' => $listing->user_id]) }}> {{ $listing->user->user_real_name }}</a></div>
            <div>Reviews rating (averaged rating): {{ (int) $listing->reviews_avg_review_rating }}</div>
            <div>Applications count: {{ $listing->applications_count }}</div>
        </div>
        <hr>

        <div class="w-full max-w-sm bg-white rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
            <a href="#">
                <img class="p-8 rounded-t-lg" src="{{ asset('storage/images/').'/'.$listingimage->listing_image_path }}" alt="product image" />
            </a>
            <div class="px-5 pb-5">
                <a href="#">
                    <h5 class="text-xl font-semibold tracking-tight text-gray-900 dark:text-white">{{ $listing->listing_name }}</h5>
                </a>
